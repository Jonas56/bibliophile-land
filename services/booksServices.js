...
const httpGetAllBooks = async (req, res) => {
  ...
  const books = await Book.findAndCountAll({
    ...
    attributes: ["id", "title", "image_link"],
    limit,
    offset,
    order: [["title", "ASC"]],
  });
...
const httpAddnewBook = async (req, res) => {
  ...
  if (body.authors) {
    ...
    return res.status(400).json({ message: "Book must belongs to an author" });
  }
  ...
  if (body.categories) {
    ...
    return res.status(400).json({ message: "Book must belongs to a category" });
  }
